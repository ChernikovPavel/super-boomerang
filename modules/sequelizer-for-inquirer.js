...
const writeTopScoreResult = async () => {
  try {
    ...
    const answerDirty = await Score.findAll({ include: [{ model: User }] });
    const ans = answerDirty.map((el) => el.get({ plain: true }));

    let best = { score: 0 };
    for (let i = 0; i < ans.length; i++) {
      if (best.score < ans[i].score) best = ans[i];
    }
    console.log(best);
    const res = { user: best.User.login, score: best.score };
    console.log('\x1b[0m'); // демаскирует
    return res;
  } catch (error) {
    console.log('\x1b[0m'); // демаскирует
